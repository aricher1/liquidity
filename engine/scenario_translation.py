...
def translate_scenario(profile, scenario, assumptions):
    ...
    for shock, value in scenario.items():
        ...
        # valuation impact
        stressed_profile.loc[stressed_profile["bucket"] == bucket, "stress_loss_pct"] += abs(value) * pass_through

        # liquidity demand
        if liquidity_demand_pct > 0:
            bucket_value = profile.loc[profile["bucket"] == bucket, "market_value"].values[0]
